...
<script>
const API_KEY = "AQ.Ab8RN6Kn6WUI14_5bcxsffDykY5ikeB5ChYzRMbydQS4";
let lastCode = "";
...
function setMode(mode, element) {
    ...
    // هنا التعديل المهم 100%
    document.getElementById('modeIcon').innerText = modeIcons;
    document.getElementById('modeTitle').innerText = `ShadowKing AI - ${modeNames}`;

    addMessage(`تم التحويل الى ${modeNames} ${modeIcons}`, 'bot');
}
...
async function sendMsg() {
    ...
    try {
        ...
        if(currentMode === 'chat') prompt += `دردش فقط. `;
        if(currentMode === 'learn') prompt += `اشرح بالتفصيل خطوة خطوة. `;
        if(currentMode === 'web' || currentMode === 'apk') prompt += `اعطي الكود كامل داخل \`\`html... \`\`. `;
        if(currentMode === 'image') prompt += `اوصف الصورة بالتفصيل. `;
        prompt += `السؤال: ${msg}`;
